...
This repository contains code, lightweight metadata, paper artifacts, and JSON/CSV/PNG results for synthetic dishonesty probing experiments.
...
4. Run `python reproduce_tables.py` to rebuild paper tables and figures from `results/*.json`.
...
- `results`: raw JSON results used by the paper.
- `paper`: manuscript versions, tables, and figures.
- `other_code`: relevant code that did not map cleanly into the requested structure.
